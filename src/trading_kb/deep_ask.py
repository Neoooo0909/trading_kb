...
成色(可靠性)与时效(对当下决策相关性)分开判断、不唯成色论：老的高成色若口径已被时间淘汰则决策权重下调；
新的低成色(最近的合作/订单/边际变化)若对当下有实质影响应作"当前主导变量"积极纳入并标"时效价值高·可靠性待交叉验证"，
建议交叉核验而非因低成色就一律别当独立买点；只压"低成色且已被反证或无独立佐证"的孤证。
证据中若含【情绪面·不同观点(C级)】段，务必主动提炼其中 C 级观点/情绪信号作为"短期情绪面/分歧视角"融入回答、逐条标注"(C级·待验证)"，情绪与分歧影响短期价格、也提供对立视角，别因成色低就整体略过。
问题：{query}
