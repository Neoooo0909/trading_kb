...
# 只放行带真实量值的 numbers。
# 2026-08-27 审核修订:① 年份分支收窄为 20[0-3]\d(原 \d{4} 把 5000/1330 这类真量值当年份,四目录抽样
# 34–128 个/400 卡被误拦);② 补 "2025年12月""2026年上半年/年底""26Q1/25H2""Q4 2025""2026-2027(年)"
# "10月底""第三季度""本周/年内""12-18 months""March 2026" 等漏拦形状(原式要求"月"后必须跟数字)。
_DATEONLY_NUM_RE = re.compile(
    r"^\s*(?:"
    r"\d{1,2}\s*月(?:[初中底末]|份)?"                                  # 9月 / 10月底
    r"|20[0-3]\d\s*年?(?:[初中底末]|E|[上下]半年|全年|年内)?"          # 2026 / 2026年 / 2026年底 / 2026年上半年 / 2026E
    r"|20[0-3]\d[-/年.]\d{1,2}(?:[-/月.]\d{1,2})?\s*(?:[日号]|月(?:底|末|初)?)?"   # 2026-08-27 / 2025年12月 / 2026.8
    r"|20[0-3]\d\s*年?\s*(?:Q[1-4]|H[12]|[一二三四]季度|[上下]半年)"    # 2026年Q1 / 2026H2 / 2026年三季度
    r"|(?:Q[1-4]|H[12])\s*(?:20)?\d{2}"                                # Q4 2025 / Q1 26 / H2 2026
    r"|Q[1-4]|[1-4]Q\d{2,4}|\d{2}[QH][1-4]|\d+H\d{2}|FY\s*\d{2,4}"    # Q3 / 3Q26 / 26Q1 / 25H2 / 1H26 / FY25
    r"|\d{1,2}\s*月\s*\d{1,2}\s*[日号]"                                 # 1月15日
    r"|\d{1,2}\s*[日号]|[一二三四]季度|第[一二三四]季度|[上下]半年|年[初中底末]|年内|本周|本月|本季度"
    r"|[一二三四五六七八九十两半]+\s*(?:个月|天|周|年|季度)"                 # 一年 / 三个月
    r"|(?:20)?\d{2}[-~至到](?:20)?\d{2}\s*年?"                        # 2026-2027 / 2026-2027年 / 26-27年
    r"|\d+[-~至到]\d+\s*(?:月|个月|天|周|年)"                            # 3-5月 / 12-18个月
    r"|\d+\s*(?:个月|天|周|年|季度)"                                   # 3个月 / 5天 / 20年(年限)
    r"|\d+(?:[-~至到]\d+)?[-\s]?(?:days?|weeks?|months?|years?|yrs?)"   # 12-18 months / 3 years
    r"|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[a-z]*\.?\s*(?:\d{1,2},?\s*)?(?:20)?\d{2}"  # March 2026
    r"|\d{1,2}/\d{1,2}/20\d{2}|20\d{2}/\d{1,2}/\d{1,2}"               # 12/07/2025
    r")\s*$", re.I)
